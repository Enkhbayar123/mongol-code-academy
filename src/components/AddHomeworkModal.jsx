...
export default function AddHomeworkModal({ isOpen, onClose, onCreated }) {
  ...
  // Use all practice problems
  const availableProblems = basicPracticeData || [];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await addDoc(collection(db, 'homeworks'), {
        title: title.trim(),
        assignedClass: 'all', // Universal assignment for all students
      ...
      setErrorMsg('Алдаа гарлаа: ' + err.message);
    } finally {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 font-sans">
      <div className="bg-[#0b1120] border border-slate-800 rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
        {/* Header */}
        <div className="px-6 py-4 border-b border-slate-800 flex items-center justify-between bg-[#080e1a]">
          <div>
            <h2 className="text-lg font-bold text-white">Шинэ гэрийн даалгавар нэмэх</h2>
            <p className="text-xs text-slate-400 mt-0.5">
              Сонгосон бодлогууд бүх сурагчдад нийтлэг гэрийн даалгавар болж харагдана
            </p>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-5">
          {errorMsg && (
            <div className="p-3 text-xs bg-rose-950/40 border border-rose-900/50 text-rose-300 rounded-xl">
              {errorMsg}
            </div>
          )}
          ...
          <div>
            ...
            <input
              type="text"
              required
              placeholder="Жишээ: 1-р долоо хоногийн даалгавар"
              value={title}
              onChange={(e) => setTitle(e.target.value)}
              className="w-full bg-[#050913] border border-slate-800 rounded-xl px-3.5 py-2.5 text-sm text-white focus:outline-none focus:border-sky-500"
            />
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-xs font-semibold text-slate-300">
                Бодлогууд сонгох ({selectedProblemIds.length} / {availableProblems.length})
              </label>
              <button
                type="button"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-60 overflow-y-auto p-2 bg-[#050913] border border-slate-800 rounded-xl">
              {availableProblems.map((problem) => {
                ...
                return (
                  <div
                    ...
                    className={`flex items-center space-x-3 p-2.5 rounded-lg cursor-pointer transition-colors text-xs select-none ${
                      checked
                  ...
                  >
                    <input
                      ...
                      checked={checked}
                      onChange={() => {}}
                      className="rounded border-slate-700 text-sky-600 focus:ring-0 pointer-events-none"
                    />
